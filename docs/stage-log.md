...
- 统一全项目文档中的边界口径，明确这是纯静态 Web 展陈版本，不接入真实登录、真实预约、数据库、CMS、后台或 API，也不收集用户信息
...
- 统一当前阶段口径为 `Stage 4B / C1 Static Structure Build` 或“原创国风志怪 IP 数字展陈宣发系统”的纯静态阶段成果，保留 README Development Stages 中的历史阶段记录
...
- 作品集表达：确认项目为原创国风志怪 IP 数字展陈宣发系统，不宣称真实游戏上线运营。
- 部署说明：确认继续按静态站点部署，无构建命令、无后端环境。

## Stage 4C：NCDA 1-C1 参赛提交收口

- 当前目标从继续扩充页面转为参赛交付收口：在现有 15 个静态页面基础上，完成三层网页结构、3 分钟展陈导览、评委浏览路径、匿名合规检查与人工整理参赛材料清单。
- `demo.html` 从旧答辩时长口径统一为 NCDA 1-C1 3 分钟展陈导览，讲述顺序调整为作品定位、首页视觉、角色展陈、剧情档案、地图寻迹、世界观档案、宣发入口、技术边界与创新价值。
- README、CODEX 与 C1 提交检查清单同步记录 `archives.html` 世界观档案库、`demo.html` 3 分钟展陈导览、15 页静态结构与最终参赛材料要求。
- 后续执行重点为参赛材料清单：作品简介图说明、展板文案、海报文案、宣讲脚本、匿名检查表与人工打包说明；本地临时整理目录不进入 Git。
- 本阶段仍不新增后端、数据库、真实登录、真实预约、上传、评论、收藏、统计、CMS、API 或大规模视觉重做。

## Stage 4D：原创国风志怪 IP 数字展陈宣发系统定位收束

- 将首页、Project 区、Demo 页、README 与阶段记录的主定位统一为“原创国风志怪 IP 数字展陈宣发系统”。
- 首页补强三层价值说明：原创 IP 数字展陈、网页化展陈路径、服务评审理解 / 作品集传播 / 早期 IP 提案。
- Project 区重构为四层评审锚点：内容展陈层、宣发导览层、评审路径层、技术落地层。
- 角色详情页补充关联剧情档案、关联地点与关联世界观条目；剧情档案页补充相关人物、地图点位与推荐继续阅读。
- 世界观档案库卡片补充出现页面与推荐下一步阅读，地图区域补充剧情与档案入口对应说明。
- `demo.html` 调整为 9 步 3 分钟展陈导览：项目定位、首页视觉、角色展陈、剧情档案、地图寻迹、世界观档案、宣发入口、静态部署与技术边界、创新价值与后续扩展。
- 本阶段仍保持纯静态边界，不引入框架、后端、数据库、登录逻辑、真实预约提交或 API，不大改既有视觉风格。
